> # ⚠️ 本文件已作废（2026-09-16）—— 请勿据此施工
>
> 本块建立在 owner 2026-09-14《裁决答复》§一-4 的「**聚合计数 + 人数阈值 k=3**」口径上。
> owner 已于 **2026-09-16 重新裁定**：roR8Pk 第 1 条的正式口径为
> 「**事项进行中只见本人立场；进入终态后全员可见**」——**按事项状态判定，不按人数**。
> 本文件的 A 方案（含 `per_person_visible` / k=3 聚合计数 / B22 限定翻转 / 摘 xfail）**整体被推翻**。
>
> ### 实际状态（核过的，不是转述）
>
> | 项 | 事实 |
...
> | 已有测试 | `tests/api/test_stance_delayed_disclosure.py`（4 条，全绿） |
> | 聚合计数实现 | **不存在**。全仓无 `per_person_visible`、无 `participant_count >= 3` 判定 |
> | 待做代码 | **无**。该口径已落地，本块不产生任何新开工单 |
>
> ### 为什么保留而不是删除
>
> 保留存档，用于追溯 09-14 那次口径的来源与推导过程。删除会让「A 方案曾经存在过」这件事失去凭据。
>
> ### 关联
>
> - 更正件：`outputs/2026-09-16-裁定更正与补定-致执行侧.md`
> - 落地记录：`outputs/2026-09-16-裁定落地执行记录.md`
> - 保留的双重代价（owner 已知情接受）：**进行中看不到队友立场**（协作透明度）/ **结案后反推保护归零**（隐私）
