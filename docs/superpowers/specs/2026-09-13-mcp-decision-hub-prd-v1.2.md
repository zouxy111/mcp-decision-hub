...
- **一次性告知**：本人开通 `can_commit` 时，Web 端必须做一次性告知，文案至少覆盖三点——① 开启后本人在本事项中的提交可由 Agent 代理、**不再逐次要求本人确认**；② 本人可随时撤销；③ 不可逆事项上代理提交会被拒绝。**可直接粘贴的文案草案见 4.3；确切措辞待 owner 定稿（见 §4.2 待办 1）。**
...
**可直接粘贴的文案草案（【待 owner 定稿】）**

以下两段为**草案**，供 owner 快速定稿。措辞、口径与是否需勾选确认均由 owner 决定；定稿前不得作为对外承诺发布。

> **【待 owner 定稿】4.3 告知义务增列句（追加在现有第三方 LLM 告知之后）**
>
> 「本事项中可能存在未经本人逐次确认的代理提交；此类内容与本人确认后提交的内容一样，会一并发送至上述第三方大模型服务商。」

> **【待 owner 定稿】参与人开通 `can_commit` 的一次性告知（弹层正文）**
>
...
### 4.1 本轮回合已关闭（owner 2026-09-13 拍板）
...
| # | 事项 | 为什么需要确认 |
|---|---|---|
| 1 | 4.3 增列句与开通一次性告知的**确切文案定稿**（本稿已给可直接粘贴的草案，均标注【待 owner 定稿】）；是否还需「我已阅读」勾选确认。 | 对外承诺，与 v1.1 附录 A.5 第 2 项同类，需 owner 与合规责任方定稿 |
...
| 3 | 悬空引用修复方案：本稿选 A（新增 §15.1）。若评审选 B（改 `hub/schemas/stance.py:3` 注释指向自身），必须**删除 §15.1**，避免双事实源。 | 方案二选一，需评审确认后收口 |
| 4 | `irreversible` 变更时**理由是否必填**；以及撤销 `can_commit` 后，历史代理提交是否需在 Web 上标注「授权已撤销」。本稿只要求保留 `acting_as` 留痕，未要求撤销标注。 | 两条均为 Q10/Q7 未覆盖的细节 |
| 5 | `agent_authority` 的开通/撤销是否限于事项未进入终态（`completed` / `cancelled`）。本稿未设额外限制，仅沿用 7.1 终态只读。 | Q7 未涉及时序限制 |
| 6 | §1.1 一句话定位是否需要改写（现句「提交经本人确认的纯文本输出」）。本稿仅在 §2.4 落实机制，未改定位句。 | 定位句属对外表述，需 owner 定 |
| 7 | 仓库事实待纠正（**非本稿契约，不影响条款**）：`hub/db/models.py:197` 与 `hub/api/stances.py:154` 的注释声称「本仓库没有迁移工具」，与 `hub/db/migrations/`（版本 1 baseline、版本 2 add_stance_check_constraints）的事实矛盾。本稿按「已有迁移机制」写（见 2.4.6）。 | 是仓库内既有的过期注释，纠正需改代码，不在本稿范围 |
